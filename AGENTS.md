...
8. Do not write the reflection, append to `LEARNING_LOG.md`, change a skill-confidence state, or update `PROGRESS.md` until the user explicitly confirms that the reflection is accurate and the checkpoint is complete.
9. After that confirmation, record the approved evidence and reflection, update only justified skill confidence, and update `PROGRESS.md` immediately to the next task—or to the next exercise when the completed task was the exercise's last task. This keeps the restart location exact between checkpoints.
