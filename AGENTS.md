...
17. Whenever the shared learning harness or an exercise changes after learner state exists, prepare a separate reusable update against clean `main` and inspect its complete candidate diff for personal progress, reflections, identity, private links, and credentials before committing or pushing it. Never copy the learner versions of `PROGRESS.md`, `SKILLS.md`, or `LEARNING_LOG.md` wholesale into `main`; apply only intentional template/schema changes from the clean starter side, and exclude learner-specific orientation or sources from `AGENTIC_AI_MAP.md`. Commit and push only the reviewed reusable files to `main`, synchronize the learner branch with that commit, and then restore or retain learner state there. Keep existing starter tags immutable; publish a new tag for a later starter version rather than moving an earlier tag.
